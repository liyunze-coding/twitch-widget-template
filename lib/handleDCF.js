...
class TwitchAuth {
	constructor(clientId, scopes, channel, sender) {
		...
		this.database = new DBStorage("twitch", "token");
	}
	...
	async saveToken(tokenData) {
		await this.database.save("twitch_token", tokenData);
	}

	async getToken() {
		let token = "";
		// 3 tries, 1 second interval, try to get token
		for (let i = 0; i < 3; i++) {
			token = await this.database.get("twitch_token");
			if (token) {
				break;
			}
	...
	setupAuthModal() {
		...
		modalOverlay.innerHTML = `
            <div id="modal-container" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 300px; text-align: center;line-height: 1.4rem; overflow-y:scroll; height: inherit;">
                <h2 style="
					display: block;
					font-size: 1.5em;
					margin-top: 0.83em;
	...
	displayNoClientId() {
		...
		modalOverlay.innerHTML = `
            <div id="modal-container" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 300px; text-align: center; overflow-y:scroll; height: inherit;">
                <h2 style="
					display: block;
					font-size: 1.5em;
					margin-top: 0.83em;
                ...
                <div style="margin-bottom: 10px; line-height: 1.4rem">
                    ...
                    Enter <b>Client ID</b> in credentials.js file
                </div>
